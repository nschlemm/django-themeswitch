...
# https://github.com/tomchristie/django-rest-framework/blob/master/rest_framework/runtests/runcoverage.py
import os
import sys

# fix sys path so we don't need to setup PYTHONPATH
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "../.."))
os.environ['DJANGO_SETTINGS_MODULE'] = 'themeswitch.runtests.settings'

from coverage import coverage


def main():
    """Run the tests for themeswitch and generate a coverage report."""
    ...
    if hasattr(TestRunner, 'func_name'):
        ...
        failures = test_runner.run_tests(['themeswitch.tests'])
    cov.stop()

    # Discover the list of all modules that we should test coverage for
    import themeswitch

    project_dir = os.path.dirname(themeswitch.__file__)
    cov_files = []
